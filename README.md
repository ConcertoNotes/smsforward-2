...
- Telegram 与飞书并行发送并分别记录投递进度，一个通道超时不会拖慢或重复发送另一个通道。
- 新消息优先于延迟重试消息，单条失败消息不会堵塞后续队列。
...
Telegram 转发要求手机网络能够直接访问 `api.telegram.org`。应用不内置代理或中转服务；在 Telegram 官方 API 不可达的网络中，Telegram 会按退避策略重试，但不会影响飞书消息的及时发送。
...
- 入队事件与配置变更会立即唤醒发送服务，空闲轮询仅作为故障兜底。
- 跳过尚未到重试时间的旧消息，并优先处理新消息，消除队首阻塞。
